...
**目标**：让外部 AI Agent 通过稳定语义接口读取、校验和修改 Shizuku 图，而不是在 Graph Editor 内维护聊天 UI。

#### 已完成

- [x] 移除硬编码回复的 AI 助手占位界面
- [x] 基于官方 MCP C# SDK 的独立 stdio Server
- [x] Unity Editor loopback 桥、项目发现文件与随机认证令牌
- [x] 图列表、节点目录、读取、校验和事务式修改工具
- [x] dry-run 与 revision 冲突保护
- [x] Codex、Claude Code、Cursor 检测与配置入口
- [x] 未知 Agent 的通用 stdio 配置

#### 后续

- [ ] 扩展变量、函数与 Blueprint Event 的专用语义操作
- [ ] 增加自动布局和更细粒度的差异预览
- [ ] 根据真实消费项目反馈扩展其他 Agent 配置器

**实现说明**：详见 [MCP 集成文档](AI_ASSISTANT_SOLUTIONS.md)
