...
namespace Editor.Generator
{
    ...
    public class UnifiedShizukuGeneratorTab
    {
        ...
        private const string VARIABLE_STORE_OUTPUT_PATH = "Assets/Scripts/Graph/Generated/RuntimeVariableStore.Generated.cs";
        ...
        private void GenerateVariableTypes()
        {
            ...
            try
            {
                ...
                // 生成 RuntimeVariableStore partial
                GenerateRuntimeVariableStorePartial(customTypes);
                ...
                _statusLabel.text = $"✅ Generated variable types, ports, and Set/Get nodes for {customTypes.Count} class(es)";
                EditorUtility.DisplayDialog("Success",
                    $"Successfully generated for {customTypes.Count} class(es):\n" +
                    $"• VariableType enum\n" +
                    $"• GraphVariable partial\n" +
                    $"• RuntimeVariableStore partial\n" +
        ...
        private void GenerateRuntimeVariableStorePartial(List<ShizukuClassInfo> customTypes)
        {
            var sb = new StringBuilder();
            ...
            sb.AppendLine();
            sb.AppendLine("/// <summary>");
            sb.AppendLine("/// RuntimeVariableStore 的自动生成扩展部分");
            sb.AppendLine("/// 包含所有自定义 ShizukuClass 类型的字典、初始化、加载和克隆");
            sb.AppendLine("/// </summary>");
            sb.AppendLine("public partial class RuntimeVariableStore");
            sb.AppendLine("{");

            if (customTypes.Count > 0)
            {
                // 字典字段
                sb.AppendLine("    // Custom type dictionaries (auto-generated)");
                foreach (var typeInfo in customTypes)
                {
                    var typeName = typeInfo.Type.FullName ?? typeInfo.Type.Name;
                    var dictName = $"Custom_{typeInfo.Type.Name}s";
                    sb.AppendLine($"    public Dictionary<string, {typeName}> {dictName};");
                }
                sb.AppendLine();

                // InitCustomTypeDictionaries
                sb.AppendLine("    partial void InitCustomTypeDictionaries()");
                sb.AppendLine("    {");
                foreach (var typeInfo in customTypes)
                {
                    var typeName = typeInfo.Type.FullName ?? typeInfo.Type.Name;
                    var dictName = $"Custom_{typeInfo.Type.Name}s";
                    sb.AppendLine($"        {dictName} = new Dictionary<string, {typeName}>();");
                }
                sb.AppendLine("    }");
                sb.AppendLine();

                // LoadCustomTypeVariable
                sb.AppendLine("    partial void LoadCustomTypeVariable(GraphVariable variable)");
                sb.AppendLine("    {");
                ...
                foreach (var typeInfo in customTypes)
                {
                    ...
                    sb.AppendLine($"            case {enumName}:");
                    sb.AppendLine($"                {dictName}[variable.GUID] = variable.{valueFieldName};");
                    sb.AppendLine($"                break;");
                }
                sb.AppendLine("        }");
                sb.AppendLine("    }");
                ...
                // CloneCustomTypeDictionaries
                sb.AppendLine("    partial void CloneCustomTypeDictionaries(RuntimeVariableStore target)");
                sb.AppendLine("    {");
                foreach (var typeInfo in customTypes)
                {
                    var typeName = typeInfo.Type.FullName ?? typeInfo.Type.Name;
                    var dictName = $"Custom_{typeInfo.Type.Name}s";
                    sb.AppendLine($"        target.{dictName} = {dictName} != null ? new Dictionary<string, {typeName}>({dictName}) : null;");
                }
                sb.AppendLine("    }");
            }
            else
            ...
            File.WriteAllText(VARIABLE_STORE_OUTPUT_PATH, sb.ToString());
            Debug.Log($"[UnifiedShizukuGenerator] Generated: {VARIABLE_STORE_OUTPUT_PATH}");
        }
        ...
        private void GenerateCustomGetVariableNodes(List<ShizukuClassInfo> customTypes)
        {
            ...
            foreach (var typeInfo in customTypes)
            {
                ...
                var dictName = $"Custom_{simpleTypeName}s";
                ...
                sb.AppendLine();
                sb.AppendLine($"    [SerializeReference]");
                sb.AppendLine($"    public {portClassName} Output = new {portClassName} {{ IsOut = true, Name = \"Value\" }};");
                sb.AppendLine();
                ...
                sb.AppendLine($"    public override void GetOutputValues()");
                sb.AppendLine($"    {{");
                sb.AppendLine($"        if (_parentGraph.VariableStore.{dictName}.TryGetValue(VariableGUID, out var value))");
                sb.AppendLine($"        {{");
                sb.AppendLine($"            Output.Value = value;");
                sb.AppendLine($"        }}");
                sb.AppendLine($"        else");
                sb.AppendLine($"        {{");
                sb.AppendLine($"            Output.Value = default;");
                sb.AppendLine($"        }}");
                sb.AppendLine($"    }}");
                sb.AppendLine();
        ...
        private void GenerateCustomSetVariableNodes(List<ShizukuClassInfo> customTypes)
        {
            ...
            foreach (var typeInfo in customTypes)
            {
                ...
                sb.AppendLine();
                sb.AppendLine($"    [SerializeReference]");
                sb.AppendLine($"    public {portClassName} Input = new {portClassName} {{ IsOut = false, Name = \"Value\" }};");
                sb.AppendLine();
                sb.AppendLine($"    [SerializeField]");
                sb.AppendLine($"    private ChainPort _nextPort = new ChainPort {{ Name = \"Next\" }};");
                sb.AppendLine();
                sb.AppendLine($"    public override string Title => GetDisplayName();");
                ...
                sb.AppendLine($"    protected override void OnExecute()");
                sb.AppendLine($"    {{");
                sb.AppendLine($"        _parentGraph.VariableStore.{dictName}[VariableGUID] = Input.Value;");
                sb.AppendLine($"    }}");
                sb.AppendLine();
                sb.AppendLine($"    protected override bool OnSelectNextNode(out string nextNodeGUID)");
                sb.AppendLine($"    {{");
